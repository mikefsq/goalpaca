...
func (c *fakeCamera) NumX() int                         { return 4 }
func (c *fakeCamera) NumY() int                         { return 3 }
...
func (c *fakeCamera) ImageFrame() (server.ImageFrame, error) {
	if !c.started {
		return server.ImageFrame{}, server.ErrValueNotSet
	}
	pix := make([]byte, 4*3*2)
...
func TestCameraImageArray(t *testing.T) {
	...
	// Before exposure the server-side ImageReady gate rejects the read with
	// InvalidOperation (ICameraV4), round-tripped through the ImageBytes envelope.
	if _, err := c.ImageArray(); !errors.Is(err, server.ErrInvalidOperation) {
		t.Fatalf("ImageArray() before exposure: want InvalidOperation, got %v", err)
	}
	...
	if frame.ElementType != server.ImgInt32 || frame.TransmissionElementType != server.ImgUInt16 {
